...
async function sendToAI(question, modelOverride = null) {
  ...
  async function sendWithRetry(contextCount) {
    try {
      ...
      // 直接通过fetch发送请求（替代Electron API）
      const fetchResponse = await fetch(apiEndpoint + `?question=${encodeURIComponent(requestData.ques)}&system=${encodeURIComponent(requestData.system)}`);
      ...
      if (responseText.includes('414') || responseText.includes('Request-URI Too Large')) {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    // 从本地存储加载所有对话
    const savedConversations = localStorage.getItem('xiaor-all-conversations');
...
// 网页版语音识别函数
async function startWebVoiceRecognition() {
  ...
  return new Promise((resolve, reject) => {
    ...
    recognition.onresult = function(event) {
      ...
      if (interimTranscript) {
        console.log('语音识别中间结果:', interimTranscript);
      }
    };
    
    recognition.onerror = function(event) {
      console.error('语音识别错误:', event.error);
      reject(getRecognitionErrorMessage(event.error));
    };
...
async function startElectronVoiceRecognition() {
  try {
    ...
    const result = await startWebVoiceRecognition();
...
function saveSettings() {
  ...
  // 更新主进程中的自启动设置
  // 网页版不支持自启动设置
  console.log('网页版不支持自启动设置');
  
  // 通知主进程关闭时直接退出设置已更改
  // 网页版不支持关闭时直接退出设置
  console.log('网页版不支持关闭时直接退出设置');
...
async function generateConversationTitle(conversationId) {
  ...
  try {
    ...
    // 直接通过fetch发送请求（替代Electron API）
    const apiEndpoint = 'https://api.jkyai.top/API/depsek3.2.php';
    const response = await fetch(apiEndpoint + `?question=${encodeURIComponent(requestData.ques)}&system=${encodeURIComponent(requestData.system)}`);
    const responseText = await response.text();
    
    if (responseText && responseText.trim()) {
      // 清理响应内容，只保留标题
      let title = responseText.replace(/^[\s\n\r]+|[\s\n\r]+$/g, '').substring(0, 20); // 去除首尾空白并限制长度
...
async function saveAllConversations() {
  try {
    // 保存所有对话到本地存储
    localStorage.setItem('xiaor-all-conversations', JSON.stringify(allConversations));
  } catch (error) {
...
function checkForUpdate() {
  showCustomAlert('检查更新', '当前已是最新版本: v1.1.2，如有更新会在此处显示。');
